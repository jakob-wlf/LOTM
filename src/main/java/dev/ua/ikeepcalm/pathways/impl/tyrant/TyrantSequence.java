...
import dev.ua.ikeepcalm.pathways.Sequence;
import org.bukkit.*;
    ...
    public static void spawnLighting(Location loc, Entity caster, double multiplier, boolean npc, boolean destruction, Integer sequence) {
        ...
        for (int j = 0; j < 12; j++) {
            ...
            for (int i = height; i > 0; i--) {
                GeneralPurposeUtil.drawDustsForNearbyPlayers(loc1, 5, 0.15, 0.15, 0.15, dust1);
                loc1.add(vector1.clone().multiply((1.0 / distance)));
            }
        }

        for (int i = 0; i < 50; i++) {
            GeneralPurposeUtil.drawDustsForNearbyPlayers(particleLoc.add(0, -1, 0), 20, 0, 0, 0, dust);
        ...
        for (Entity entity : loc.getWorld().getNearbyEntities(particleLoc, damageRadius, damageRadius / 2f, damageRadius)) {
            if (GeneralPurposeUtil.testForValidEntity(entity, caster, true, true)) {
                ...
                GeneralPurposeUtil.drawParticlesForNearbyPlayers(Particle.ELECTRIC_SPARK, entity.getLocation(), 100, 0.1, 0.1, 0.1, .75);
            }
