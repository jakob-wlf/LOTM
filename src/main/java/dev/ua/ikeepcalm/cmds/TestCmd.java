...
public class TestCmd implements CommandExecutor {
    ...
    public boolean onCommand(@NonNull CommandSender s, @NonNull Command command, @NonNull String label, @NonNull String[] args) {
        ...
        if (args[0].equalsIgnoreCase("characteristic")) {
            p.getInventory().addItem(Plugin.instance.getCharacteristic().getCharacteristic(GeneralPurposeUtil.parseInt(args[2]), args[1], "§" + args[3]));
        }

        if (args[0].equalsIgnoreCase("recipe")) {
            p.getInventory().addItem(Plugin.instance.getRecipe().getRecipeForSequence(Plugin.instance.getPotions().get(0), GeneralPurposeUtil.parseInt(args[1])));
        }
